...
class HourglassModel():
	...
	def _train(self, nEpochs = 10, epochSize = 1000, saveStep = 500, validIter = 10):
		...
		with tf.name_scope('Train'):
			...
			self.resume['loss'] = []
			self.resume['err'] = []
			self.resume['accur_val'] = []
			self.resume['loss_val'] = []
			self.resume['err_val'] = []
			...
			for epoch in range(nEpochs):
				...
				# 	accuracy_pred = self.Session.run(self.joint_accur, feed_dict = {self.img : img_valid, self.gtMaps: gt_valid})
				# 	accuracy_array += np.array(accuracy_pred, dtype = np.float32) / validIter
				# print('--Avg. Accuracy val =', str((np.sum(accuracy_array) / len(accuracy_array)) * 100)[:6], '%' )
				# self.resume['accur_val'].append(accuracy_pred)
				# self.resume['err_val'].append(np.sum(accuracy_array) / len(accuracy_array))
				# valid_summary_val = self.Session.run(self.test_op, feed_dict={self.img : img_valid, self.gtMaps: gt_valid})
				# self.test_summary.add_summary(valid_summary_val, epoch)
				# self.test_summary.flush()
	...
	def _init_weight(self):
		...
		print('*********** number of trainable weights:', 
		          self.Session.run(tf.reduce_sum([tf.reduce_prod(v.shape) for v in tf.trainable_variables()])) )
